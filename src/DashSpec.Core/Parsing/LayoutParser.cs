...
internal static class LayoutParser
{
    ...
    /// <summary>Tab-level bracket board: layout { [ Q E ] [ T F ] }.</summary>
    public static LayoutBoardDefinition ParseBoard(TokenReader reader)
    {
        reader.Expect(TokenKind.LBrace);
        reader.SkipNewlines();
        var board = ParseBoardRows(reader);
        reader.Expect(TokenKind.RBrace);
        return board;
    }

    /// <summary>Bracket rows until EOF or closing brace (for .dashlayout modules).</summary>
    public static LayoutBoardDefinition ParseBoardRows(TokenReader reader)
    {
