window.dashSpecMatrix = {
  ...
  fitCellSize(xCount, yCount, availableWidth, availableHeight, gapPx, yLabelCol = 144, visibleRows = null) {
    const gap = gapPx ?? 2;
    const xLabelRow = 28;
    const chrome = 16;
    ...
    const scrollRowCap = visibleRows > 0 ? visibleRows : growThreshold;
    ...
    cellW = Math.min(52, Math.max(preferredCellW, cellW));
    let cellH = Math.floor((gridH - gap * (yCount + 1)) / Math.max(1, yCount));
    cellH = Math.min(48, Math.max(preferredCellH, cellH));
    return { cellW, cellH };
  },
