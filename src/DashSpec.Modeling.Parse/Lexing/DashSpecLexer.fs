...
module DashSpecLexer =
    ...
    let private trySkipSlashSlashComment (text: string) (i: byref<int>) =
        if i + 1 >= text.Length || text.[i + 1] <> '/' then false
        else
            i <- i + 2
            skipToEndOfLine text &i
            true

    let private trySkipBlockComment (text: string) (i: byref<int>) (atLineStart: byref<bool>) =
        if i + 1 >= text.Length || text.[i + 1] <> '*' then false
        else
            i <- i + 2
            ...
            while i < text.Length && not closed do
                if text.[i] = '\r' || text.[i] = '\n' then
                    ...
                    i <- i + 1
            if closed then true
            else raise (DashSpec.Modeling.Core.DashSpecParseException("Unterminated block comment (/* … */)."))
    ...
    let private readString (text: string) (i: byref<int>) start =
        ...
        if i >= text.Length then
            raise (DashSpec.Modeling.Core.DashSpecParseException("Unterminated string literal."))
        i <- i + 1
    ...
    let private readMultilineString (text: string) (i: byref<int>) start =
        ...
        else raise (DashSpec.Modeling.Core.DashSpecParseException("Unterminated multiline string literal (\"\"\")."))
    ...
    let private readHexColor (text: string) (i: byref<int>) start =
        ...
        if hexLen <> 3 && hexLen <> 6 then
            raise (DashSpec.Modeling.Core.DashSpecParseException($"Invalid hex color at position {start}. Use #rgb or #rrggbb (not at line start as comment — use // or /* */)."))
    ...
    let tokenize (text: string) : IReadOnlyList<Token> =
        ...
        while i < text.Length do
            if Char.IsWhiteSpace text.[i] then
                ...
                if trySkipBlockComment text &ii &als then
                    i <- ii; atLineStart <- als
                elif atLineStart && trySkipSlashSlashComment text &ii then
                    i <- ii
                else
                    raise (DashSpec.Modeling.Core.DashSpecParseException($"Unexpected '/' at position {i}. Use // line comments at line start or /* */ block comments."))
            else
                let start = i
                match text.[i] with
                ...
                | '.' ->
                    if i + 1 < text.Length && text.[i + 1] = '.' then
                        ...
                        raise (DashSpec.Modeling.Core.DashSpecParseException($"Unexpected '.' at position {i}. Use '..' for date ranges."))
                | '-' ->
                    let relStart = i
                    i <- i + 1
                    ...
                    if i <= relStart + 1 then
                        raise (DashSpec.Modeling.Core.DashSpecParseException($"Invalid relative day at position {relStart}. Use form -Nd, e.g. -7d."))
                    ...
                    if i + 1 < text.Length && text.[i + 1] = '[' then
                        ...
                        while j < text.Length && not foundEnd do
                            if text.[j] = ']' && j + 1 < text.Length && text.[j + 1] = ']' then
                                ...
                                j <- j + 1
                        if j >= text.Length then raise (DashSpec.Modeling.Core.DashSpecParseException("Unterminated [[ raw block."))
                ...
                | c when isIdentStart c ->
                    ...
                    atLineStart <- false
                | c -> raise (DashSpec.Modeling.Core.DashSpecParseException($"Unexpected character '{c}' at position {i}.", i))
